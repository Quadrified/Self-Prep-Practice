...
const FirebaseSignIn = props => {
  ...
  const onAnonymousLogin = () => {
    auth()
      .signInAnonymously()
      .then(signInData => {
        console.log('User signed in anonymously ID<<<', signInData.user.uid);
        const userID = signInData.user.uid;
        const database = firebase
          .app()
          .database(
      ...
      {user ? (
        <>
          ...
          {/* {isSignedInWithGoogle ? (
            ...
            <AnonymousView userInfo={signedInData} onSignOut={onSignOut} />
          )} */}
        </>
      ) : (
        <>
